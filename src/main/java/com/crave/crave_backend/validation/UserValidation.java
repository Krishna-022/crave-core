		...
		if (userList.size() > 0) {
			...
			for (int i = 0; i < userList.size(); i++) {
				...
				if (contactNumber.equals(user.getContactNumber())) {
		            messageList.add(String.format(ErrorMessageConstants.ENTITY_CONFLICT, entity, contactNumber));
		        }
				if (email.equals(user.getEmail())) {
		            messageList.add(String.format(ErrorMessageConstants.ENTITY_CONFLICT, entity, email));
		        }
			}
